...
async function testWithStrongPassword() {
  console.log('🔄 Testing with Strong Password...');
  
  try {
    // Step 1: Try to create user with stronger password
    ...
    if (signUpError) {
      console.log('ℹ️ Sign up error:', signUpError.message);
      
      // Try sign in with original password
      console.log('🔍 Trying to sign in with original password...');
      ...
      if (signInError) {
        ...
        console.log('🔍 Trying to sign in with new password...');
        ...
        if (signInError2) {
          console.log('❌ Sign in with new password failed:', signInError2.message);
          console.log('💡 User may need to be created manually in Supabase Dashboard');
        } else {
          ...
          await testProfileOperations(signInData2.user?.id);
        }
      } else {
...
testWithStrongPassword().catch(console.error);
