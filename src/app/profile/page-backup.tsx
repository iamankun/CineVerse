...
export default function ProfilePage() {
  ...
  const fetchUserProfile = async () => {
    try {
      console.log("🔍 [PROFILE DEBUG] Starting profile fetch...");
      console.log("🔍 [PROFILE DEBUG] Environment:", process.env.NODE_ENV);
      console.log("🔍 [PROFILE DEBUG] URL:", window.location.href);
      console.log("🔍 [PROFILE DEBUG] User agent:", navigator.userAgent);
      
      // Check Supabase client configuration
      console.log("🔍 [PROFILE DEBUG] Supabase URL:", process.env.NEXT_PUBLIC_SUPABASE_URL ? "Set" : "Not set");
      console.log("🔍 [PROFILE DEBUG] Supabase Key:", process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ? "Set" : "Not set");
      
      // Check if service worker is active
      if ('serviceWorker' in navigator) {
        const registration = await navigator.serviceWorker.ready;
        console.log("🔍 [PROFILE DEBUG] Service Worker active:", !!registration.active);
        console.log("🔍 [PROFILE DEBUG] Service Worker scope:", registration.scope);
      }
      
      const { data: { user }, error: authError } = await supabase.auth.getUser();
      
      console.log("🔍 [PROFILE DEBUG] Auth result:", { 
        user: user ? { id: user.id, email: user.email } : null,
        authError: authError?.message 
      });
      
      if (!user || authError) {
        console.log("🔍 [PROFILE DEBUG] No user found or auth error, redirecting to login");
        console.log("🔍 [PROFILE DEBUG] Auth error details:", authError);
        
        // Check if we have a session
        const { data: { session } } = await supabase.auth.getSession();
        console.log("🔍 [PROFILE DEBUG] Current session:", session ? "Exists" : "None");
      ...
      console.log("🔍 [PROFILE DEBUG] User authenticated:", { id: user.id, email: user.email });
      setUser(user);

      // Fetch profile data with error handling for missing columns
      console.log("🔍 [PROFILE DEBUG] Fetching profile data...");
      
      let profile = null;
      let profileError = null;
      
      try {
        console.log("🔍 [PROFILE DEBUG] Querying profiles table for user:", user.id);
        ...
        console.log("🔍 [PROFILE DEBUG] Profile query completed:", {
          profile: profile ? "Found" : "Not found",
          error: profileError?.message,
          errorCode: profileError?.code
        });
      } catch (error: any) {
        console.error("🔍 [PROFILE DEBUG] Profile query failed:", error);
        profileError = error;
      }
      ...
      console.error("🔍 Debug: Full error in fetchUserProfile:", {
      ...
      addToast({
        title: "Lỗi",
        description: error.message || "Không thể tải thông tin trang cá nhân",
        color: "danger",
      });
    } finally {
      setLoading(false);
